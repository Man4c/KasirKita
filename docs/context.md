...
- **Implementasi Sistem Multi-Tenant & Lisensi Toko (`Plan 36`)**:
  ...
  - *Status Fase 5 (Selesai)*:
    ...
    - Memperbarui `mobile/src/screens/SettingsScreen.js` pada seksi "IDENTITAS TOKO & LISENSI" dengan kartu status lisensi interaktif, badge real-time (TRIAL, PRO AKTIF, KEDALUWARSA), hitung mundur sisa hari, tanggal kedaluwarsa dinamis, dan tombol aktivasi.
    - Menjadikan durasi lisensi pada modal aktivasi (`LicenseActivationModal.js`) dan kartu pengaturan dinamis sesuai paket serial key (`1_month` ➔ 1 Bulan s/d tgl, `1_year` ➔ 1 Tahun, atau `lifetime` ➔ Seumur Hidup).
  ...
  - *Pemisahan Akun Superadmin SaaS vs Pemilik Toko & Fix Relasi Lisensi*:
    ...
    - **Pembersihan Antarmuka Login (`Login.jsx`)**: Menghapus seksi tombol demo siap pakai agar form autentikasi web tampil profesional, bersih, dan siap untuk produksi tanpa membocorkan kredensial akun di tampilan publik.
...
- **Implementasi Mobile Smart Hybrid Backup-Restore & Sinkronisasi Top-Down Preferensi Cloud - Fase 3, 4, 5, & 6 Plan #24 (`backupService.js`, `BackupRestoreModal.js`, `SettingsScreen.js`, `offlineStorage.js`, `StoreSetting.php`)**:
  - Menyelesaikan seluruh fase Plan #24 (Cloud Sync Preferences & Local Data Backup-Restore) dari backend hingga frontend mobile:
    1. *Modul `backupService.js` (Smart Hybrid Backup)*:
       ...
       - **Multi-Platform File Export & Import Resilient**: Mendukung download virtual Blob di Web dan `expo-file-system/legacy` + `expo-sharing` di Android/iOS native. Pada Android, proses import dilengkapi *Dual-Engine File Reader* (`FileSystem.readAsStringAsync` dengan fallback ke React Native global `fetch(asset.uri).then(r => r.text())`) serta `copyToCacheDirectory: false` untuk mengatasi pencegahan izin Scoped Storage Android/Expo Go (`isn't readable`). Backend `ProductController` juga mendukung `is_active=all` sehingga ekspor Cloud API menyertakan katalog produk lengkap.
       - **Sinkronisasi Pemulihan ke Database Cloud (`POST /settings/restore`)**: Proses pemulihan data tidak hanya memperbarui cache lokal `AsyncStorage`, melainkan juga menyinkronkan seluruh master data (kategori, unit, produk, pelanggan, pemasok, pajak, diskon) ke database server backend (`POST /api/settings/restore`). Kategori atau produk yang sebelumnya dihapus (*soft-deleted*) otomatis di-restore kembali (`$model->restore()`) sehingga tidak tertimpa/hilang saat layar master memanggil API cloud.
...
- **Perancangan Modul Master Pengguna & Staf Kasir Mobile (React Native) (`plans/260905-32-mobile-master-pengguna/plan.md`)**:
  ...
  - **Phase 3 Selesai**: Pembuatan dua modal native slide-up bottom sheet: `UserFormModal.js` (formulir pendaftaran/edit staf dengan selector kartu role `Kasir` & `Pemilik (Owner)`, input password toggle mata, switch aktif, danger zone delete akun, dan sticky footer) dan `ResetPasswordModal.js` (bottom sheet khusus reset kata sandi/PIN kasir dengan verifikasi konfirmasi dan banner notifikasi pencabutan sesi aktif).
  - **Phase 4 Selesai**: Pembuatan layar utama `UserManagementScreen.js` dengan header standar KasirKita (22px, tombol kembali 36dp, refresh 36dp, subjudul metrik staf & omset), 4 tab filter role seimbang (`Semua`, `Kasir`, `Owner`, `Nonaktif`), search input dengan debouncing 350ms dan tombol clear, mode offline warning banner, empty state informatif, dan Extended Pill FAB `[+] Tambah Staf`.
  - **Phase 5 Selesai**: Registrasi rute `user_management` di `App.js` dengan proteksi peran owner (`user?.role === 'owner'`), penambahan launcher tile ke-8 `Staf Kasir` (`UserCheck`, aksen Rose `#f43f5e`) di `DashboardActionHub.js` & `DashboardScreen.js`, serta shortcut navigasi `Kelola Staf & Kasir` di `SettingsScreen.js` pada seksi Akun Pengguna.
  - **Phase 6 Selesai**: Audit kualitas deterministik Impeccable (0 defect / pelanggaran pada seluruh 4 file modul), verifikasi penuh font $\ge 12$px, flexbox pairing defense, touch targets $\ge 44$dp, dan kompilasi bundle Metro Expo web bundler sukses tanpa warning/error (2.352 modul). Modul plan berstatus `done`.
...
- **Penyelesaian Penuh Master Pemasok & Distributor di Mobile (Phase 6 Audit & Multi-Platform Build, Plan #31 100% Selesai)**:
  - Menyelesaikan seluruh audit kualitas, kepatuhan WCAG, dan pengujian build multi-platform:
    ...
    2. *Pemeriksaan Linter Desain*: Menjalankan skrip detector Impeccable (`detect.mjs`) pada seluruh file komponen pemasok dengan hasil **0 defect**.
    3. *Verifikasi Sintaks*: Semua modul JavaScript lulus pemeriksaan `node -c` (0 syntax error).
    4. *Pengujian Build Multi-Platform*: Menjalankan `npx expo export --platform web` pada proyek mobile, berhasil bundel 2.347 modul tanpa error dalam ~2,8 detik.
    5. *Status*: Seluruh fase (Phase 1-6) pada Plan #31 tuntas (`completed`).
...
- **Implementasi Phase 3 Master Pajak & Biaya Layanan di Mobile (`TaxCardItem.js`, `TaxManagementScreen.js`, Plan #29)**:
  - Mengimplementasikan komponen visual kartu item dan layar utama berstandar Defensive UI Impeccable:
    ...
    2. *Layar Utama (`TaxManagementScreen.js`)*: Tiga kartu metrik ringkasan (Total Pajak, Biaya Layanan, Aktif Digunakan), pencarian debounced 350ms, segmented filter tabs (*Semua*, *Pajak*, *Biaya Layanan*), filter status (*Semua*, *Aktif*, *Nonaktif*), FlatList tervirtualisasi, mode offline banner, dan FAB Tambah.
    3. *Kepatuhan Impeccable & Defensive UI*: Flexbox pairing (`min-w-0 truncate` vs `shrink-0 whitespace-nowrap`), readability floor (seluruh teks $\ge 12$px), serta lolos audit linter `detect.mjs` (0 defect). Phase 3 berstatus `completed`.
...
- **Penyejajaran Vertikal Menyeluruh Badges, Pills, dan Metrics Box (`ProductManagementScreen.js`, `PromoManagementScreen.js`, `TransactionHistoryScreen.js`, `ProductCardItem.js`, `CategoryCardItem.js`, `UnitCardItem.js`, `PromoCardItem.js`, `ProductGrid.js`, `LandscapeRegisterPanel.js`)**:
  - Menyelesaikan masalah teks dan ikon yang tidak tepat di tengah (*off-center / bottom-heavy*) pada seluruh badge status, filter chips, pill opsi, dan kotak metrik di layar Android:
    1. *Kartu Produk & Riwayat Transaksi*:
       - Menyejajarkan badge status riwayat (`statusBadgeSuccess`, `statusBadgeCancelled`, `statusBadgeOffline`), metode pembayaran (`methodBadge`), tombol sinkronisasi (`syncNowBtn`), dan chips filter riwayat (`filterChip`).
       - Menyejajarkan badge kategori produk (`categoryBadge`) dan badge nonaktif (`inactiveBadge`) di kartu produk.
    2. *Kartu Promosi*:
       - Menyejajarkan badge kode kupon (`couponBadge`), badge tipe diskon (`typeBadge`), dan pill status aktif/nonaktif/kadaluarsa (`statusPillActive`, `statusPillInactive`, `statusPillExpired`).
    3. *Terminal Kasir POS & Landscape Panel*:
       - Menyejajarkan filter kategori kasir (`catChip`, `catChipCompact`) di `ProductGrid.js`.
       - Menyejajarkan pill opsi cepat kasir (`regQuickPill`, `regQuickPillCompact`) di `LandscapeRegisterPanel.js`.
    4. *Penyelarasan Standar Typography & Geometri*:
       - Seluruh elemen badge & pill kini memiliki ketinggian eksplisit (`height`), `justifyContent: 'center'`, `alignItems: 'center'`, serta penambahan `includeFontPadding: false` dan `textAlignVertical: 'center'` pada setiap elemen `Text`.
  - Lolos verifikasi linter Impeccable (`detect.mjs` $\rightarrow$ 0 defects) dan build Expo Web (`npx expo export --platform web`).
...
- **Resolusi Anti-Patterns & Defect Impeccable pada Master Satuan (`UnitManagementScreen.js`, `UnitCardItem.js`, `UnitFormModal.js`)**:
  - Menyelesaikan 12 AI tells dan 11 quality issues yang terdeteksi pada audit browser Master Satuan:
    ...
    2. *Eliminasi Dark Mode with Glowing Accents*: Menghilangkan colored glow neon (`#9333ea`) pada FAB dan tombol utama. Background diganti dengan warna standar KasirKita Rose Red (`#e11d48`) dan bayangan dinetralkan (`shadowColor: '#000000'`).
    3. *Resolusi Flat Type Hierarchy*: Meningkatkan skala tipografi halaman (`screenTitle` dinaikkan dari 18px ke **22px** `Poppins_700Bold` dengan `letterSpacing: -0.4`, metrik ringkasan **20px**, modal/empty title 16px, nama satuan 15px, input/button 13px, dan label/badge 12px), menghasilkan rasio hierarki tipografi $\ge \mathbf{1.83:1}$ (mengatasi rasio sempit 1.5:1).
    4. *Perbaikan Kontras Ekstrem (WCAG AA/AAA)*:
       - Tombol Utama (`createBtnHeader`, `emptyCreateBtn`, `fab`, `submitBtn`): Teks putih `#ffffff` di atas `#e11d48` menghasilkan rasio kontras **6.3:1** (lulus WCAG AAA).
       - Badge Penggunaan Kosong & Tombol Hapus Nonaktif: Mengganti warna teks `#71717a` (3.1:1) menjadi **`#a1a1aa`** (rasio kontras **4.8:1** di atas latar `#27272a`, memenuhi standar WCAG AA $\ge 4.5:1$).
       - Eliminasi Flattening DOM Web: Menghilangkan `rgba(192, 132, 252, 0.12)` yang memicu flattening 1.0:1 pada DOM web, digantikan background solid `#27272a` dengan teks `#e4e4e7`.
  - Lolos audit linter Impeccable (`detect.mjs` $\rightarrow$ 0 defects) dan lolos verifikasi build Expo Web (`npx expo export --platform web`).
