...
| `arrange [target]` | Enhance | Alias for layout: fix spacing, layout rhythm, alignment | [reference/layout.md](reference/layout.md) |
...
- `teach` aliases `init`. `arrange` aliases `layout`. `craft` is a deprecated alias for ordinary new-work and adds nothing. `shape` owns task discovery, then enters new-work only for visual-world and surface-concept decisions.
